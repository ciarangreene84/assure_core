...
{
    public class BenefitsRepositoryTests : AbstractConfigurationRepositoryTests<IBenefitsRepository, Benefit>
    {
        public BenefitsRepositoryTests()
        {
     
        }

        [Fact]
        public async void InsertBenefitAsync()
        {
            var newItem = A.New<TestBenefit>();
            using (var dbConnection = await _dbConnectionFactory.OpenUserConnectionAsync(_userId))
            {
                var items = (await _repository.GetAsync<TestBenefit>()).ToList();
                newItem.BenefitId = items.Max(candidate => candidate.BenefitId) + 1;
            ...
            using (var dbTransaction = await _dbConnectionFactory.BeginUserTransactionAsync(_userId))
            {
                try
                {
                    var result = await _repository.InsertAsync<TestBenefit>(dbTransaction, newItem);
        ...
        public async void UpdateBenefitAsync()
        {
            ...
            using (var dbTransaction = await _dbConnectionFactory.BeginUserTransactionAsync(_userId))
            {
                try
                {
                    var insertedItem = await _repository.InsertAsync<TestBenefit>(dbTransaction, newItem);
                    Assert.NotNull(insertedItem);
                    Assert.NotEqual(0, insertedItem.BenefitId);

                    insertedItem.Name = $"{newItem.Name} (Updated: {DateTime.Now})";
                    var result = await _repository.UpdateAsync<TestBenefit>(dbTransaction, insertedItem);
                    dbTransaction.Commit();
                    Assert.True(result);
                }
                catch
        ...
        public async void DeleteBenefitAsync()
        {
            ...
            using (var dbTransaction = await _dbConnectionFactory.BeginUserTransactionAsync(_userId))
            {
                try
                {
                    ...
                    var result = await _repository.DeleteAsync<TestBenefit>(dbTransaction, insertedItem);
                    dbTransaction.Commit();
                }
                catch
                {
        ...
        {
            var result = await _repository.GetAsync<TestBenefit>();
            Assert.NotNull(result);
        }
    }

    public class TestBenefit : Benefit
    {
